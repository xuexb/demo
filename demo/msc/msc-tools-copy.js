...
(function($, msc){
    var count = 0,
        isIe6 = msc.tools.browser.isIe6,
        _expando = 'msc_copy_'+ $.now(),
    copy = msc.tools.copy = function( config, str ){
        ...
        config = $.extend({
            id: "",
            str: "美食天下 - 让吃更美好",
            path: 'http://static.meishichina.com/v6/img/mscCopy.swf',
        ...
        $.each(config.id, function(){
            ...
            if(! this._copy){
                ...
                //打上标识,并实例
                this._copy = _expando + (count++);
                copy._list[this._copy] = new Class(this._copy, this, config);
            } else {
                copy._get(this._copy) && copy._get(this._copy).setText(config.str);
            }
        });
    ...
    Class.prototype = {
        ...
        _bind: function(){
            ...
            //如果有msc.event对象
            if(msc.event){
                msc.event.resize.add(this.id, function(){
                    that._resize();
                });
            } else {
